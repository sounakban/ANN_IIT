...
emb1 = embedding(net, input_dim=len(embeddings), output_dim=len(embeddings[0]), trainable=True, name="EmbeddingLayer")
emb2 = embedding(net, input_dim=len(POS_vectors), output_dim=len(POS_vectors[0]), trainable=True, name="POSLayer")
net = tf.concat([emb1, emb2], 2)
#net = merge([emb1, emb2], mode='concat', axis=2)
print("Shape after embeddings : ", net.get_shape().as_list())
branch1 = conv_1d(net, 150, 2, padding='valid', activation='relu', regularizer="L2", weight_decay=3)
print("Shape after CNN1 : ", branch1.get_shape().as_list())
branch2 = conv_1d(net, 150, 3, padding='valid', activation='relu', regularizer="L2", weight_decay=3)
print("Shape after CNN2 : ", branch2.get_shape().as_list())
branch3 = conv_1d(net, 150, 4, padding='valid', activation='relu', regularizer="L2", weight_decay=3)
print("Shape after CNN3 : ", branch3.get_shape().as_list())
branch4 = conv_1d(net, 150, 5, padding='valid', activation='relu', regularizer="L2", weight_decay=3)
