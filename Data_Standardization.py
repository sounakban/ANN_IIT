...
def oneHot_to_standard(doc_vectors, trig_vectors, tag_vec, sent_len=10, posit_window_len=5, append_posit_vec=True):

	if posit_window_len%2 == 0:
		raise ValueError("Please enter a  odd number as windows size, to ensure even distribution around trget word.")
	doc_split, trig_split, tag_split = split(doc_vectors, trig_vectors, tag_vec, sent_len)
	ret_doc = []
	ret_trig = []
...
def split(doc_vectors, trig_vectors, tag_vec, sent_len):
	...
	if len(doc_vectors) > sent_len:
		...
		for i in range(len(doc_vectors)):
			...
			tag_split.append(tag_vec[start:end])
